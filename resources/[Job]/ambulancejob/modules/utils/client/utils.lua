...
function utils.addRemoveItem(type, item, quantity)
    ...
    TriggerServerEvent("ambulancejob:removAddItem", data)
end
...
function utils.useItem(item, value)
    ...
    data.item = item
    data.value = value
    TriggerServerEvent("ambulancejob:useItem", data)
end

function utils.getItem(name)
    local item = lib.callback.await('ambulancejob:getItem', false, name)
...
RegisterNetEvent('ambulancejob:showNotification', utils.showNotification)
