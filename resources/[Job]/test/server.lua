...
RegisterNetEvent("ambulancejob:updateDeathStatus", function(death)
    local source = source
    local data = {}
    data.target = source
...
RegisterNetEvent("ambulancejob:revivePlayer", function(data)
    local source = source
    ...
    if data.targetServerId < 1 or #(GetEntityCoords(sourcePed) - GetEntityCoords(targetPed)) > 4.0 then
        ...
        TriggerClientEvent('ambulancejob:healPlayer', tonumber(data.targetServerId), dataToSend)
    end
end)

RegisterNetEvent("ambulancejob:healPlayer", function(data)
    local source = source
    if not Framework.hasJob(source, emsJobs) or not source or source < 1 then return end
    ...
    if data.injury then
        TriggerClientEvent('ambulancejob:healPlayer', tonumber(data.targetServerId), data)
    else
        data.anim = "medic"
        TriggerClientEvent("ambulancejob:playHealAnim", source, data)
        data.anim = "dead"
        TriggerClientEvent("ambulancejob:playHealAnim", data.targetServerId, data)
    end
end)

RegisterNetEvent("ambulancejob:createDistressCall", function(data)
    local source = source
    ...
    for i = 1, #players do
        ...
        if Framework.hasJob(id, emsJobs) then
            TriggerClientEvent("ambulancejob:createDistressCall", id, playerName)
        end
    end
end)

RegisterNetEvent("ambulancejob:callCompleted", function(call)
...
RegisterNetEvent("ambulancejob:removAddItem", function(data)
    local source = source
...
RegisterNetEvent("ambulancejob:useItem", function(data)
...
RegisterNetEvent("ambulancejob:removeInventory", function()
    local source = source
...
RegisterNetEvent("ambulancejob:putOnStretcher", function(data)
    if not player[data.target].isDead then return end
    TriggerClientEvent("ambulancejob:putOnStretcher", data.target, data.toggle)
end)

RegisterNetEvent("ambulancejob:togglePatientFromVehicle", function(data)
    if not player[data.target].isDead then return end

    TriggerClientEvent("ambulancejob:togglePatientFromVehicle", data.target, data.vehicle)
end)

lib.callback.register('ambulancejob:getDeathStatus', function(source, target)
    return player[target] and player[target] or Framework.getDeathStatus(target or source)
end)

lib.callback.register('ambulancejob:getData', function(source, target)
    local data = {}
...
lib.callback.register('ambulancejob:getDistressCalls', function(source)
    return distressCalls
end)

lib.callback.register('ambulancejob:openMedicalBag', function(playerId)
...
lib.callback.register('ambulancejob:getMedicsOniline', function(source)
    local count = 0
    local players = GetPlayers()
...
if ox_inventory then
    lib.callback.register('ambulancejob:getItem', function(source, name)
...
lib.versionCheck('Arius-Development/ambulancejob')
